...
import { storedLead } from '../../contexts/QuestionnaireContext';
...
    const validaEtapa = React.useCallback(async (schema: z.Schema, data: unknown) => {
...
    const validatePagamento = React.useCallback(async () => {
...
        return true;
    }, [dadosPagamento, formaSelecionada, validaEtapa]);
    const handleCloseModal = () => {
        setModal(false);
        navigate('/login');
    };
    const handleIniciarAssinatura = React.useCallback(async () => {
        if (isLoading) return; 
...
            clearTimeout(timer);
        };
    }, [formaSelecionada, Modal, isLoading, handleIniciarAssinatura]);
