...
function TelaCadastro() {
 ...
 const [dadosSolicitante, setDadosSolicitante] = useState<DadosSolicitante>({
    // Se seus types (v2) usam 'nomeCompleto', ajuste aqui.
    // Se usam 'nome' e 'sobrenome' (como na v1), isto está correto.
 ...
 const handleFinish = async () => {
  const isValid = await validateCurrentStep();
  if (isValid) {
      // 1. Recria o objeto completo com TODOS os dados
   const dadosCompletosCadastro = {
      ...
      // 2. Envia 'cadastroData' E 'plano' no state, como a TelaPagamento (v1) espera
   navigate('/cadastro/pagamento', { 
            state: { 
                cadastroData: dadosCompletosCadastro, 
                plano: planoPreSelecionado || 'essencial' 
            } 
        });
  }
 };
