...
function TelaCadastro() {
 ...
 const planoSelecionado = (location.state?.planoPreSelecionado as PlanoEscolhido) || 'essencial';
 ...
 const handleFinish = async () => {
  ...
  const dadosCompletosCadastro = {
   dadosSolicitante: { ...dadosSolicitante, plano: planoSelecionado },
   dadosEmpresa: {
    ...dadosEmpresa,
    lead: diagnosticoData 
  },
   dadosSenha
  };
  navigate('/cadastro/pagamento', {
   state: { cadastroData: dadosCompletosCadastro, plano: planoSelecionado }
  });
 };
  ...
  <div className="cadastro-wrapper">
   <div className="cadastro-container">
    ...
    <div className={`form-buttons ${step > 1 ? 'form-buttons--between' : 'form-buttons--end'}`}>
     ...
     {step < 4 ? (
      <button type="button" className="button-primary" onClick={nextStep} disabled={isLoadingCpf || isLoadingCnpj}>Próximo</button>
     ) : (
      <button type="button" className="button-primary" onClick={handleFinish} disabled={isLoadingCpf || isLoadingCnpj}>
