...
export const step1Schema = z.object({
  ...
  setorPrincipal: z.enum(["Indústria", "Serviços", "Comércio / Varejo", "Tecnologia / Startups", "Educação / Cultura"], { message: "Selecione o setor." }),
  porteEmpresa: z.enum(["Startup", "PME", "Grande Empresa"], { message: "Selecione o porte." }),
  localizacao: z.string().min(1, { message: "Por favor, selecione uma localização." }),
  numeroColaboradores: z.enum(["ate10", "11a30", "30a100", "acima100", "acima500"], { message: "Selecione o número de colaboradores." }),
});
...
// --- Step 3: Objeto puro sem superRefine
...
export const step4Schema = z.object({
  faixaInvestimento: z.enum(["ate10k", "10ka50k", "acima50k"], { message: "Selecione uma faixa de investimento." }),
  decisorContratacao: z.enum(["CEO/Diretor", "RH/T&D", "Marketing", "Outro"], { message: "Selecione um decisor." }),
  preferenciaTreinamento: z.enum(["Presenciais", "Online", "Hibridos"], { message: "Selecione uma preferência de treinamento." }),
  aberturaInovacao: z.enum(["1", "2", "3", "4", "5"], { message: "Classifique a pergunta de acordo." }),
  importanciaDesenvolvimento: z.enum(["1", "2", "3", "4", "5"], { message: "Classifique a pergunta de acordo." }),
  importanciaSoftSkills: z.enum(["1", "2", "3", "4", "5"], { message: "Classifique a pergunta de acordo." }),
  importanciaCultura: z.enum(["1", "2", "3", "4", "5"], { message: "Classifique a pergunta de acordo." }),
  importanciaImpacto: z.enum(["1", "2", "3", "4", "5"], { message: "Classifique a pergunta de acordo." }),
  implementouProjetosInovadores: z.enum(["Sim", "Nao"], { message: "Selecione uma opção." }),
  tempoInicio: z.enum(["Imediatamente", "ate3meses", "6mesesoumais"], { message: "Selecione um tempo para iniciar." }),
});

// --- Step 5
export const step5Schema = z.object({
  preferenciaFormato: z.enum(["Vídeo", "Áudio", "Leitura", "Prática"],{ message: "Selecione o formato preferido de aprendizagem." }),
  duracaoPreferida: z.enum(["Longos", "Curtos"],{ message: "Selecione a duração preferida dos treinamentos." }),
  contextoTrabalho: z.enum(["Computador", "Deslocamento", "Manual"],{ message: "Selecione o principal contexto de trabalho da equipe." }),
  formaAprendizagem: z.enum(["Vídeos", "Áudios", "Leitura"],{ message: "Selecione o formato atual de aprendizagem." }),
  modoAprendizagem: z.enum(["Individual", "Grupo"],{ message: "Selecione o formato atual de aprendizagem." }),
  tempoDisponivel: z.enum(["Menosde1", "1a3h", "3a5h", "Maisde5"],{ message: "Informe o tempo disponível semanal para treinamentos." }),
});

// --- Step 6
export const step6Schema = z.object({
  treinamentosRecentes: z.enum(["Sim", "Nao"],{ message: "Informe se houve treinamentos formais nos últimos 12 meses." }),
  beneficiosSuficientes: z.enum(["Sim", "Parcialmente", "Nao"],{ message: "Informe se os benefícios atuais atendem à equipe." }),
  transparenciaPromocoes: z.enum(["0", "1", "2", "3", "4", "5"], { message: "Classifique a transparência nas promoções." }),
  acoesSociaisAmbientais: z.enum(["Sim", "Parcialmente", "Nao"],{ message: "Classifique a transparência nas promoções." }),
  iniciativasCulturais: z.enum(["Sim", "Parcialmente", "Nao"],{ message: "Informe se a empresa incentiva cultura, arte ou criatividade." }),
  apoioHobbies: z.enum(["Sim", "Parcialmente", "Nao"],{ message: "nforme se há espaço para hobbies e talentos pessoais." }),
